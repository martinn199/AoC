    ...
    while (fgets(line, sizeof(line), fPointer) != NULL) {
        ...
        while(move_done < number_crates){
            if(crates[j][move_from] == ' '){
                ...
                while(crates[j][move_from] != ' '){
                    if(y >= number_crates){
                        break;
                    }
                    char_move_from[y] = crates[j][move_from];
                    crates[j][move_from] = ' ';
                    y++;
                    j++;
                }
            ...
            /*Insert the crates at the disered positions by looping from the end of crates.*/
            /*The crates is inserted at the first empty place in the disered column*/
            f = MAX_SIZE - 1;
            g = strlen(char_move_from) - 1;
            do{
                if (crates[f][move_to] != ' ' && crates[f - 1][move_to] == ' '){
                    crates[f - 1][move_to] = char_move_from[g];
                    move_done++;
                    g--;
                }
                f--;
