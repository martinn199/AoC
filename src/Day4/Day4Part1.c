...
int main(){
    ...
    while(fgets(data, DATA_SIZE, fPointer) !=NULL){
        ...
        /*Check if there is a pair that fully contains the other pair*/
        if((number1 <= number3 && number2 >= number4) || (number3 <= number1 && number4 >= number2)){
            /*Sum these pairs*/
            total_assignments = total_assignments + 1;;
        }
        /*Do not include a pair that not fully contains the other pair*/
        else{
            total_assignments = total_assignments;
        }
    }
    /*Close the file*/
    /*Print the total assignements*/
